...
- Financial capacity: ${yourCompanyProfile.financialCapacityScore}/100 — factor this into how long they can sustain this move
...
export function buildOrchestratorPrompt(triggerEvent, allFinalResponses, yourCompanyProfile, scenarioContext = {}) {
  ...
  const ycCapacityInterp = yourCompanyProfile.financialCapacityScore >= 70
    ? 'high — can sustain aggressive or prolonged competitive moves'
    : yourCompanyProfile.financialCapacityScore >= 40
  ...
  const ycIntentSorted = Object.entries(yourCompanyProfile.strategicIntentVector ?? {})
    ...
    .map(([dim, score]) => {
      ...
      return `  ${labels[dim] ?? dim}: ${(score * 100).toFixed(1)}%`;
    }).join('\n');
...
- Financial Capacity: ${yourCompanyProfile.financialCapacityScore}/100 → ${ycCapacityInterp}
- Strategic Intent:\n${ycIntentSorted}
