...
export async function runSimulation(scenario) {
  ...
  const yourCompany = {
    ...
    rdSpendPct: yourCompanyInput.rdSpendPct ?? 0,
    lastThreePriceMoves: yourCompanyInput.lastThreePriceMoves ?? [],
    marketShareTrend: yourCompanyInput.marketShareTrend ?? 'stable',
    headcountTrend: yourCompanyInput.headcountTrend ?? 'flat',
    ceoPriorityStatement: yourCompanyInput.ceoPriorityStatement ?? '',
    recentNewsSignals: yourCompanyInput.recentNewsSignals ?? [],
    regulatoryConstraints: yourCompanyInput.regulatoryConstraints ?? '',
  ...
  const pseudoYourCompanyProfile = {
    ...yourCompanyProfile,
    name: 'Your Company',
    strategicMove: (yourCompanyProfile.strategicMove ?? '').replaceAll(yourCompany.name, 'Your Company'),
    context: (yourCompanyProfile.context ?? '').replaceAll(yourCompany.name, 'Your Company'),
  };
